...
void runExample7(const std::string& rootDir, size_t resolutionX,
    int numberOfFrames, const std::string& format, double fps) {
    // Build solver
    auto solver =
        FlipSolver3::builder()
        .withResolution({ resolutionX * 2, 2 * resolutionX, resolutionX })
        .withDomainSizeX(2.0)
        .makeShared();
    ...
    auto sphere = Sphere3::builder()
        .withCenter({ 0.1, domain.height() * 0.9, domain.depth() * 0.5 })
        .withRadius(0.1)
        .makeShared();
    ...
    auto box1 =
        Box3::builder()
        .withLowerCorner({ 0, 0, 0 })
        .withUpperCorner({ domain.width() * 0.2, domain.height() * 0.8, domain.depth() })
        .makeShared();

    auto box2 =
        Box3::builder()
        .withLowerCorner({ domain.width() * 0.2, 0, 0 })
        .withUpperCorner({ domain.width() * 0.4, domain.height() * 0.6, domain.depth() })
        .makeShared();

    auto box3 =
        Box3::builder()
        .withLowerCorner({ domain.width() * 0.4, 0, 0 })
        .withUpperCorner({ domain.width() * 0.6, domain.height() * 0.4, domain.depth() })
        .makeShared();

    auto box4 =
        Box3::builder()
        .withLowerCorner({ domain.width() * 0.6, 0, 0 })
        .withUpperCorner({ domain.width() * 0.8, domain.height() * 0.2, domain.depth() })
        .makeShared();

    auto box5 =
        Box3::builder()
        .withLowerCorner({ domain.width() * 0.8, 0, 0 })
        .withUpperCorner({ domain.width(), 0, domain.depth() })
        .makeShared();

    auto boxSet = ImplicitSurfaceSet3::builder()
        .withExplicitSurfaces({ box1, box2, box3, box4, box5 })
        .makeShared();

    auto collider =
        RigidBodyCollider3::builder().withSurface(boxSet).makeShared();
    ...
    auto emitterSet = ParticleEmitterSet3::builder()
        .withEmitters({ emitter1 })
        .makeShared();

    solver->setParticleEmitter(emitterSet);

    // Print simulation info
    printf("Running example 7 (water cascade with FLIP)\n");
    printInfo(solver);
...
int main(int argc, char* argv[]) {
    ...
    case 6:
        ...
        break;
    case 7:
        runExample7(outputDir, resolutionX, numberOfFrames, format, fps);
        break;
    default:
        std::cout << toString(parser) << '\n';
        exit(EXIT_FAILURE);
