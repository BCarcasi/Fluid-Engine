...
#define JET_TESTS(testSetName) 
    class testSetName : public ::testing::Test { 
     ...
     protected: 
        void SetUp() override { 
            _testCollectionDir 
                = pystring::os::path::join( 
                    JET_TESTS_OUTPUT_DIR, "testingDir"); 
        ...
        template <typename T> 
        void saveData( 
            const jet::ConstArrayAccessor1<T>& data, 
        ...
        template <typename T> 
        void saveData( 
            const jet::ConstArrayAccessor2<T>& data, 
            const std::string& name) { 
            std::string filename = getFullFilePath(name); 
            unsigned int dim[2] = { 
        ...
        template <typename T> 
        void saveData( 
            const jet::ConstArrayAccessor3<T>& data, 
            const std::string& name) { 
            std::string filename = getFullFilePath(name); 
            unsigned int dim[3] = { 
...
#define JET_BEGIN_TEST_F(testSetName, testCaseNmae) 
    TEST_F(testSetName, testCaseNmae) { 
        createTestDirectory("testingDir");
